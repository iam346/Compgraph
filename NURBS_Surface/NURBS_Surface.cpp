...
/**
 * centrális fetítés inicializálása
 **/
void initCentralProj(matrix4x4& C, GLdouble s){
        C.initIdenM();
 ...
 * Wtv mátrix
 **/
void initWtv(matrix4x4 &Wtv, point2D &Wmin, point2D &WDist, point2D &Vmin, point2D &VDist){
        Wtv.initIdenM();

        Wtv.matrix[0][0] = VDist.getX() / WDist.getX();
        Wtv.matrix[1][1] = VDist.getY() / WDist.getY();
        Wtv.matrix[0][3] = - Wmin.getX()*(Wtv.matrix[0][0]) + Vmin.getX();
        Wtv.matrix[1][3] = -Wmin.getY()*(Wtv.matrix[1][1]) + Vmin.getY();
}

/**
 * Inhomogénre vált egy pontot
 **/
void toInHomogen(point4D &point){
        GLdouble H = point.getH();

        point.setX(point.getX() / H);
        point.setY(point.getY() / H);
        point.setZ(point.getZ() / H);
        point.setH(point.getH() / H);
}
